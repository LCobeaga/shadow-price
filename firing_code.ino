...
#define PUSH_RET_PIN 8   //sets pusher return switch to pin 8
...
Button pushret(PUSH_RET_PIN, 25, true, true);   //creates pusher return sensor button
...
void setup() {
  ...
  pushret.begin();          //same but for pusher return switch
}                                       
...
void loop() {
  ...
  if(!trigger.isPressed()&&pushret.isPressed()){   //checks to see if trigger is pressed and pusher is in return position 
    digitalWrite(ACTUATOR_PIN,LOW);                //closes current flow to solenoid
  }
